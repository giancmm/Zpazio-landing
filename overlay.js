...
// === Cursor: auto-contraste según fondo ===
(function(){
  try{
    const bulb = document.getElementById('cursor-bulb');
    if(!bulb) return;
    ...
    const effectiveBg = (el)=>{
      ...
      return {r:255,g:255,b:255,a:1}; // fallback claro
    };
    const onMove = (e)=>{
      const el = document.elementFromPoint(e.clientX, e.clientY);
      const bg = effectiveBg(el);
      const L = relLum(bg);
      if(L > 0.78){ bulb.classList.add('on-light'); }
      else{ bulb.classList.remove('on-light'); }
      bulb.style.left = e.clientX + 'px';
      bulb.style.top = e.clientY + 'px';
    };
    window.addEventListener('mousemove', onMove, { passive: true });
  }catch(e){
    console && console.warn && console.warn('Cursor contrast error:', e);
  }
})();
